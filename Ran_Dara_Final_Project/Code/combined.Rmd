...
data %>%
  ...
  geom_point(alpha=.3) + 
  labs(x="Percent of adults with less than a high school diploma, 2013-17")+geom_smooth(method="lm",se=FALSE)

data %>%
  ggplot(aes(y=`Birth Rate`, x=`high_school`)) +
  geom_point(alpha=.3) + 
  labs(x="Percent of adults with a high school diploma only, 2013-17")+geom_smooth(method="lm",se=FALSE)

data %>%
  ggplot(aes(y=`Birth Rate`, x=`some_college`)) +
  geom_point(alpha=.3) + 
  labs(x="Percent of adults completing some college or associate's degree, 2013-17")+geom_smooth(method="lm",se=FALSE)

data %>%
  ggplot(aes(y=`Birth Rate`, x=`college`),) +
  geom_point(alpha=.3) + 
  labs(x="Percent of adults with a bachelor's degree or higher, 2013-17")+geom_smooth(method="lm",se=FALSE)

data %>%
  ggplot(aes(y=`Birth Rate`, x=`UnemploymentRate`)) +
  geom_point(alpha=.3) + 
  labs(x="Unemployment Rate")+geom_smooth(method="lm",se=FALSE)
```
